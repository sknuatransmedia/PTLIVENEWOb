...
#import "StateNewsViewController.h"
#import "Constant.h"
@interface StateNewsViewController ()

@end

@implementation StateNewsViewController
...
{
    [self funcStateNewsAndURL];
}
#pragma mark ---------- Get Top News -------
-(void)funcStateNewsAndURL
{
    NSURL *strUrl = [NSURL URLWithString:[NSString stringWithFormat:@"%@",PT_TAMILNADU_URL]];
...
-(void)func_reponseLatestNewsData:(NSData *)response
{
    ...
    if (response!=nil)
    {
        ...
        for (int i=0; i<arr_containCount; i++)
        {
            ...
            tempDictStateNews = [NSDictionary dictionaryWithObjectsAndKeys:str_title,@"title",str_description,@"description",str_entryid,@"entry_id",str_date,@"strdate",str_imageSmall,@"smallimage",str_imageMedium,@"mediumimage",str_imageLarge,@"largeimage",str_video,@"videos", nil];
            
            [muarrStateNews addObject:tempDictStateNews];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *strArrVal = [NSString stringWithFormat:@"%@",[[muarrStateNews objectAtIndex:indexPath.row] valueForKey:@"mediumimage"]];
    
    [imgmenu sd_setImageWithURL:[NSURL URLWithString:strArrVal]
               placeholderImage:[UIImage imageNamed:@"Water146x169.png"]];
    
    
    lblmenu.text = [NSString stringWithFormat:@"%@",[[muarrStateNews objectAtIndex:indexPath.row] valueForKey:@"title"]];
